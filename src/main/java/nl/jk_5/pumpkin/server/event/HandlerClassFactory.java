...
class HandlerClassFactory implements HandlerFactory {
    ...
    private final File outDir = new File(new File("generated"), "handler-class");
    private final boolean outputGenerated = System.getProperty("pumpkin.event.debug.handler", "false").equalsIgnoreCase("true");
    ...
    public byte[] generateClass(Class<?> objectClass, Method method, Class<?> eventClass, boolean ignoreCancelled, String className) {
        ...
        {
            mv = cw.visitMethod(ACC_PUBLIC, "handle", "(Lnl/jk_5/pumpkin/api/event/Event;)V", null, null);
            mv.visitCode();
            mv.visitVarInsn(ALOAD, 0);
        ...
        if(outputGenerated){
            outDir.mkdirs();
            File dest = new File(outDir, className + ".class");
