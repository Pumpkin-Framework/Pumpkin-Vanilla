...
public class MapZone extends AbstractZone {

    private static final Logger logger = LogManager.getLogger();

    private final int mappackId;

    public MapZone(DatabaseZone zoneInfo) {
        super(zoneInfo);

        mappackId = zoneInfo.getMappack().getId();

        logger.info("Loaded MapZone:");
    ...
    public boolean isInZone(@Nullable Player player, @Nullable User user) {
        if(player != null && player.isOnline()){
            assert player.getMap() != null;
            return player.getMap().getMappack().getId() == mappackId;
        }else{
            return false;
        }
    }
