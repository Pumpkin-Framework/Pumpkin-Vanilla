...
import nl.jk_5.pumpkin.server.util.event.reflect.AccessorFirstStrategy;
import nl.jk_5.pumpkin.server.util.event.reflect.Property;
import nl.jk_5.pumpkin.server.util.event.reflect.PropertySearchStrategy;
...
class ClassGenerator {
    ...
    private final File classOutDir = new File(new File("generated"), "event-class");
    private final File factoryOutDir = new File(new File("generated"), "event-factory");
    private final boolean outputGeneratedClass = System.getProperty("pumpkin.event.debug.class", "false").equalsIgnoreCase("true");
    private final boolean outputGeneratedFactory = System.getProperty("pumpkin.event.debug.factory", "false").equalsIgnoreCase("true");
    ...
    public byte[] createClass(final Class<?> type, final String name, final Class<?> parentType) {
        ...
        if(outputGeneratedClass){
            classOutDir.mkdirs();
            File dest = new File(classOutDir, name + ".class");
    ...
    public byte[] createFactory(final Class<?> type, final String name) {
        ...
        byte[] out = cw.toByteArray();
        if(outputGeneratedFactory){
            factoryOutDir.mkdirs();
            File dest = new File(factoryOutDir, name + ".class");
            FileOutputStream fos = null;
            try{
                fos = new FileOutputStream(dest);
                fos.write(out);
