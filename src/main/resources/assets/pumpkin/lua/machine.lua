...
local function bootstrap()
  local code = libbios.load()
  if code and #code > 0 then
    local script, reason = load(code, "=script", "t", sandbox)
    if script then
      return coroutine.create(script), {n=0}
    end
    error("failed loading script: " .. reason, 0)
  end
  error("no script found", 0)
end
